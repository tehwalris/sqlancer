...
public class FirebirdJoin implements Node<FirebirdExpression> {
    ...
    public static List<Node<FirebirdExpression>> getJoins(
        ...
        while (tableList.size() >= 2 && Randomly.getBooleanWithRatherLowProbability()) {
            ...
            case LEFT:
                joinExpressions
                        .add(FirebirdJoin.createLeftOuterJoin(leftTable, rightTable, joinGen.generatePredicate()));
                break;
            case RIGHT:
                joinExpressions
                        .add(FirebirdJoin.createRightOuterJoin(leftTable, rightTable, joinGen.generatePredicate()));
                break;
            default:
